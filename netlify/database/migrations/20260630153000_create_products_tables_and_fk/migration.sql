...
INSERT INTO "products" ("name", "slug", "description", "image_url", "price", "category_id", "stock", "sort_order") VALUES
  ('Stoneground Breakfast Blend', 'stoneground-breakfast-blend', 'A balanced pantry staple for slow mornings and busy counters.', 'https://images.unsplash.com/photo-1509042239860-f550ce710b93?auto=format&fit=crop&w=1200&q=80', 18.75, (SELECT "id" FROM "categories" WHERE "slug" = 'pantry'), 24, 1),
  ('Charcoal Linen Basket', 'charcoal-linen-basket', 'Structured storage with a quiet woven finish.', 'https://images.unsplash.com/photo-1586023492125-27b2c045efd7?auto=format&fit=crop&w=1200&q=80', 42.30, (SELECT "id" FROM "categories" WHERE "slug" = 'home-goods'), 11, 2),
  ('Pressed Citrus Gift Set', 'pressed-citrus-gift-set', 'A bright boxed set for hosts, neighbors, and overdue thank-yous.', 'https://images.unsplash.com/photo-1513201099705-a9746e1e201f?auto=format&fit=crop&w=1200&q=80', 31.40, (SELECT "id" FROM "categories" WHERE "slug" = 'gifts'), 17, 3)
ON CONFLICT ("slug") DO NOTHING;
